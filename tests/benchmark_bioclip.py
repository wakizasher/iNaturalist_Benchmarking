...
def setup_bioclip():
    ...
    # CPU optimization for Ryzen 5 7600x
    torch.set_num_threads(12) # 6 core x 2 threads = 12 threads
    torch.backends.mkldnn.enabled = True # Intel MKL-DNN

    print("Optimized for AMD Ryzen 5 7600X: Using 12 threads")
    print("MKL-DNN optimization enabled: {torch.backends.mkldnn.enabled}")

    # Load BioCLIP model and components
...
def load_ground_truth_data(csv_path, dataset_name):
...
def create_flower_species_prompts():
    flower_species = [
        ...
        "a photo of Matricarial chamomilla",
        "a photo of Leucanthemum vulgare"
    ]

    print("Flower species fo identification: ")
    for species in flower_species:
        print(f" - {species}")
    return flower_species

def test_single_image(model, preprocess_val, tokenizer, device, image_path, flower_species):
    try:
...
def process_dataset(model, preprocess_val, tokenizer, device, df, flower_species, dataset_name):
    ...
    for idx, row in df.iterrows():
        ...
        # Get prediction
        predicted_species, confidence, all_probs = test_single_image(
...
def analyze_results(filtered_results, unfiltered_results, filtered_accuracy, unfiltered_accuracy,
    ...
    if unfiltered_errors > filtered_errors:
        print(f"Unfiltered dataset had {unfiltered_errors - filtered_errors} more processing errors")
...
def main():
    ...
    try:
        ...
        filtered_df = load_ground_truth_data(filtered_csv, "Filtered Dataset")
        unfiltered_df = load_ground_truth_data(unfiltered_csv, "Unfiltered Dataset")
        ...
        print("=" * 50)
        print(f"Total experiment time: {format_time(total_experiment_time)}")
